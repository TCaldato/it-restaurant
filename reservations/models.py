...
class Booking(models.Model):
    ...
    class AppointmentTime(datetime.time, models.Choices):
        ...
        # Define choices for appointment times using class variables
    ...
    # Define fields for the Booking model
    ...
    )  # Integer field with max value validation
    start_time = models.TimeField(
        choices=AppointmentTime.choices,
        null=False,
        blank=False,
        default=AppointmentTime.PM_1800,
    )  # Time field with choices for appointment times

    class Meta:
        """Order bookings by time"""

        ordering = ["date", "start_time"]  # Order bookings by date and start_time

    def __str__(self):
        return str(self.user)  # Return a string of the Booking object
